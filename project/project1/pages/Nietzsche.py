...
niche = get_chat_service(CURRENT_TARGET)
...
for message in st.session_state[SESSION_KEY]:
    avatar = "👤" if message["role"] == "user" else CURRENT_TARGET.getAvatar()
...
if prompt := st.chat_input("심연을 들여다볼 질문을 던지시오..."):
    ...
    with st.chat_message("assistant", avatar="🧔🏻‍♂️"):
        with st.spinner("니체가 자신의 사상을 펼치고 있습니다...."):
            ...
            # RAG 로직 호출
            response_text, (state, source_text) = niche.get_response(prompt, st.session_state[SESSION_KEY])
